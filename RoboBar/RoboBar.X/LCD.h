 ...
 * File:   LCD.h
 * Author:  Cory W. Hodge
 * Comments:    Module for LCD 16x2 display, 5x8 dot, 4 bit mode
 * Revision history:  8/1/19
 */
// This is a guard condition so that contents of this file are not included
// more than once.  
#ifndef XC_HEADER_LCD
#define	XC_HEADER_LCD
...
/*
 * Method for displaying DRINKS_LIST array Drinks by index
 *  @global master_index
 *  @global *DRINKS_LIST[];
 */
void Lcd_Display_Drink(int index){
    
    // capture name of drink
    // * this is first element string in nested array
    //char * drink = DRINKS_LIST[master_index];
    
    Lcd_Clear();
    Lcd_Set_Cursor(1,1);
    Lcd_Write_String(*DRINKS_LIST[index]);
    
}

/**
 *  Confirm Drink Text Display
 */
void Lcd_Confirm_Choice(){
    
    
    Lcd_Clear();
    Lcd_Set_Cursor(1,1);
    Lcd_Write_String("Make ?");
    Lcd_Set_Cursor(2,1);
...
/**
 *  Screen to acknowledge next ingredient
 */
void Lcd_Adding_Ingredient(char * Ingredient){
    
    Lcd_Clear();
    Lcd_Set_Cursor(1,1);
    Lcd_Write_String("Adding: ");
    Lcd_Set_Cursor(2,1);
    Lcd_Write_String(Ingredient);
